...
char *generarCadenaXRemplazada(char *funcion, char *x, char *y);
char *generarCadena(char *funcion, char *x, char *y);
...
char *espacios(int n);
Fila *llenarTabla(Fila *Tabla, char funcion[], long *rInicial, long *rFinal, long *incremento);
/**
 * Declaracion de la funcion principal
 * **/
int main()
{
    //348X5-784X2+6 10 90 10
    char funcion[250];
    long rInicial;
    long rFinal;
    long incremento;
    mostrarPantallaPrincipal(funcion, &rInicial, &rFinal, &incremento);
    printf("\n%s, %li, %li, %li\n",funcion, rInicial, rFinal, incremento);
    Fila *tabla = llenarTabla(tabla, funcion, &rInicial, &rFinal, &incremento);
...
Fila *llenarTabla(Fila *tabla, char funcion[], long *rInicial, long *rFinal, long *incremento)
{
    long ix = 0;
    long iy = 0;
    for (ix = *rInicial; ix <= *rFinal; ix += *incremento)
    {
        char *cadena;
        ...
        //
        sprintf(x, "%li", ix);
        iy = resolverCadenaPolinomica(funcion, ix);
        sprintf(y, "%li", iy);
        cadena = generarCadenaXRemplazada(funcion, x, y);
...
char *generarCadenaXRemplazada(char funcion[], char *x, char *y)
{
    ...
    while (funcion[i] != 0)
    {
        ...
        char *exp = malloc(10 * sizeof(char));
